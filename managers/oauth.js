...
module.exports = (app) => {
	...
	//http://localhost:5595/account/api/public/account?accountId=VanillaBot10&accountId=VanillaBot11&accountId=VanillaBot12&accountId=VanillaBot13&accountId=VanillaBot14&accountId=VanillaBot15
	...
	app.get('/account/api/public/account/', (req, res) => {
		try {
			...
			if (`${req.query.accountId}`.startsWith("VanillaBot")) { displayName = "VanillaBot" }
			...
			req.query.accountId.forEach(accId => {
				var dn = accId
				if (`${accId}`.startsWith("VanillaBot")) { dn = "VanillaBot" }
				response.push({
					id: accId,
					displayName: dn,
					externalAuths: {}
